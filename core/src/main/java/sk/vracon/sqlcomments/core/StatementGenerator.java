...
public class StatementGenerator {
     ...
     *            parameter manages usage of generated keys during PreparedStatement creation
     */
    public PreparedStatement createPreparedStatement(Connection connection, Statement statement, StatementConfiguration configuration, boolean useGeneratedKeys) throws StatementException {
        ...
        String jdbcSql = generateJdbcStatement(statement, configuration.parameterMap(), configuration.parametersAcceptingNull(), jdbcParams, configuration.offset(), configuration.limit());
        ...
        try {
            ...
            Set<String> keysToGenerate = configuration.primaryKey(); 
            if (useGeneratedKeys && keysToGenerate != null && keysToGenerate.size() == 0) {
                // Auto generate keys
            ...
            } else if (useGeneratedKeys && keysToGenerate != null && keysToGenerate.size() > 0) {
                // Return specified keys
                jdbcStatement = connection.prepareStatement(jdbcSql, keysToGenerate.toArray(new String[0]));
            } else {
                // Use default
     ...
     * {@link #createPreparedStatement(Connection, Statement, StatementConfiguration, boolean)} are not enough. It returns generated
     ...
     * 
     * @see #generateStatement(Statement, Map, Set, Long, Long)
     */
