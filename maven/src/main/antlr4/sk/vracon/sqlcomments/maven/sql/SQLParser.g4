 ...
 *
 * Based on BNF notation from http://savage.net.au/SQL/sql-2003-2.bnf.html
 *
 * Thanks to Apache Tajo for base version.
 * Original version downloaded from https://github.com/apache/incubator-tajo/tree/master/tajo-core/tajo-core-backend/src/main/antlr4/org/apache/tajo/engine/parser
 */
...
as_clause
  : (AS)? Identifier
  | (AS)? DOUBLE_QUOTE Identifier DOUBLE_QUOTE
  ;
...
in_predicate_value
  ...
  | LEFT_PAREN variable_placeholder RIGHT_PAREN
  ;
...
pattern_matching_predicate
  ...
  | f=row_value_predicand pattern_matcher variable_placeholder
  ;
...
sort_specifier
  ...
  | replacement_identifier order=order_specification? null_order=null_ordering?
  | replacement_identifier replacement_identifier null_order=null_ordering?
  | replacement_identifier replacement_identifier replacement_identifier
  ;
...
insert_statement
  ...
  | INSERT (OVERWRITE)? INTO table_name (LEFT_PAREN column_name_list RIGHT_PAREN)? VALUES LEFT_PAREN row_value_predicand (COMMA row_value_predicand)* RIGHT_PAREN
  ;
...
set_clause
  ...
  | set_target EQUAL update_source
  ;
...
multiple_column_assignment
  : set_target_list EQUAL assigned_row
  ;
