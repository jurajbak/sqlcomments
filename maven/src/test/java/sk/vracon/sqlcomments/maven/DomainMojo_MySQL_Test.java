...
public class DomainMojo_MySQL_Test extends AbstractDomainMojoTest {
	...
	public static void generateDatabase() throws Exception {
		...
		databaseContainer = new MySQLContainerProvider().newInstance();
		...
		// Create database
		ScriptUtils.executeSqlScript(connection, new ClassPathResource("/employees-test-database/scripts/Employees - MySQL.sql"));

		// Close connection
		connection.close();
	}

	public DomainMojo_MySQL_Test() {
		super(DATABASE, MySQLDialect.class.getName(), JDBC_DRIVER, databaseContainer.getPassword(), databaseContainer.getUsername(), databaseContainer.getJdbcUrl());
	}
}
