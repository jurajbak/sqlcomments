...
public class DomainMojo_MariaDB_Test extends AbstractDomainMojoTest {
	...
	public static void generateDatabase() throws Exception {
		...
		databaseContainer = new MariaDBContainerProvider().newInstance();
		...
		// Create database
		ScriptUtils.executeSqlScript(connection, new ClassPathResource("/employees-test-database/scripts/Employees - MariaDB.sql"));

		// Close connection
		connection.close();
	}

	public DomainMojo_MariaDB_Test() {
		super(DATABASE, MariaDBDialect.class.getName(), JDBC_DRIVER, databaseContainer.getPassword(), databaseContainer.getUsername(), databaseContainer.getJdbcUrl());
	}
}
